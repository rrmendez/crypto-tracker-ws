...
export interface AccountValidatedEmailConfig extends BaseEmailConfig {
  userName: string;
}

@Injectable()
export class AccountValidatedEmailBuilder extends BaseEmailBuilder<AccountValidatedEmailConfig> {
  constructor(
  ...
  build(config: AccountValidatedEmailConfig): EmailContent {
    ...
    const subject = this.i18n.t('emails.account-validated.subject', {
      lang,
      args: {
        appName,
      },
    ...
    const text = `Congratulations! Your account on ${appName} has been successfully validated.`;
