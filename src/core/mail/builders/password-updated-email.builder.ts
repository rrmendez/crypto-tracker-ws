...
export interface PasswordUpdatedEmailConfig extends BaseEmailConfig {
  userName: string;
}

@Injectable()
export class PasswordUpdatedEmailBuilder extends BaseEmailBuilder<PasswordUpdatedEmailConfig> {
  constructor(
  ...
  build(config: PasswordUpdatedEmailConfig): EmailContent {
    ...
    const subject = this.i18n.t('emails.password-updated.subject', {
      lang,
      args: {
        appName,
      },
    ...
    const text = `We confirm that the password for your ${appName} account has been successfully changed.`;
